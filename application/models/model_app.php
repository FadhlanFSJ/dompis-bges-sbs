...
//////////////////////////////////////////////////////////////////////////////////Bagian Admin//////////////////////////////////////////////////////////////////////////////////
    ...
    public function all_ticket()
    {
        ...
        // Mengubah durasi waktu dari detik ke format jam:menit:detik
        foreach ($result as &$row) {
            $durasi_waktu = gmdate("H:i:s", $row['durasi_waktu']);
            $row['durasi_waktu'] = $durasi_waktu;
        }
        return $query;
    }

    public function get_durasi() {
      // Misalkan Anda memiliki tabel dengan nama 'waktu' dan dua kolom 'waktu_mulai' dan 'waktu_selesai'
      $query = $this->db->query("SELECT TIMESTAMPDIFF(SECOND, tanggal, last_update) AS durasi FROM ticket");
      $result = $query->row();
      
      // Durasi waktu dalam detik
      $durasi_detik = $result->durasi;

      // Konversi durasi waktu ke format jam:menit:detik
      $durasi = gmdate("H:i:s", $durasi_detik);

      return $durasi; // Format output durasi waktu (jam:menit:detik)
  }


    public function all_ticket_harian()
    {
        //Query untuk mendapatkan semua ticket berdasarkan inputan HARI INI dengan diurutkan berdasarkan tanggal tiket dibuat
        $query = $this->db->query("SELECT A.progress,A.id_ticket,A.status, A.tanggal, A.last_update, A.tanggal_solved, 
                                  A.id_kondisi, A.deadline,A.teknisi, A.problem_summary,A.filefoto, B.nama_sub_kategori,
                                  C.nama_kategori,D.nama, F.nama_dept, G.nama_kondisi, G.warna, G.waktu_respon,H.lokasi, 
                                  I.nama_jabatan, K.nama AS nama_teknisi, 
                                  TIMESTAMPDIFF(SECOND, A.tanggal, A.tanggal_solved) AS durasi_waktu
    ...
    public function all_ticket_datin()
    {
        //Query untuk mendapatkan semua ticket dengan diurutkan berdasarkan tanggal tiket dibuat
        $query = $this->db->query("SELECT A.id_ticket, A.status, A.tanggal, A.last_update, A.id_kondisi, A.deadline, A.teknisi,
                                   ...
                                   LEFT JOIN jabatan I ON I.id_jabatan = D.id_jabatan
                                   LEFT JOIN karyawan K ON K.nik = A.teknisi
                                   WHERE A.id_kondisi IN(1,2,3)
                                   ORDER BY A.tanggal DESC");
        return $query;
    }

    public function all_ticket_datin_k1()
    {
        //Query untuk mendapatkan semua ticket dengan diurutkan berdasarkan tanggal tiket dibuat
        $query = $this->db->query("SELECT A.id_ticket, A.status, A.tanggal, A.last_update, A.id_kondisi, A.deadline, A.teknisi,
                                   A.problem_summary, A.filefoto, B.nama_sub_kategori, C.nama_kategori, 
                                   ...
                                   LEFT JOIN jabatan I ON I.id_jabatan = D.id_jabatan
                                   LEFT JOIN karyawan K ON K.nik = A.teknisi
                                   WHERE A.id_kondisi IN(1) AND MONTH(tanggal)=MONTH(CURRENT_DATE)
                                   ORDER BY A.tanggal DESC");
        return $query;
    }

    public function all_ticket_datin_k2()
    {
        //Query untuk mendapatkan semua ticket dengan diurutkan berdasarkan tanggal tiket dibuat
        $query = $this->db->query("SELECT A.id_ticket, A.status, A.tanggal, A.last_update, A.id_kondisi, A.deadline, A.teknisi,
                                   A.problem_summary, A.filefoto, B.nama_sub_kategori, C.nama_kategori, 
                                   ...
                                   LEFT JOIN jabatan I ON I.id_jabatan = D.id_jabatan
                                   LEFT JOIN karyawan K ON K.nik = A.teknisi
                                   WHERE A.id_kondisi IN(2) AND MONTH(tanggal)=MONTH(CURRENT_DATE)
                                   ORDER BY A.tanggal DESC");
        return $query;
    }

    public function all_ticket_datin_k3()
    {
        //Query untuk mendapatkan semua ticket dengan diurutkan berdasarkan tanggal tiket dibuat
        $query = $this->db->query("SELECT A.id_ticket, A.status, A.tanggal, A.last_update, A.id_kondisi, A.deadline, A.teknisi,
                                   A.problem_summary, A.filefoto, B.nama_sub_kategori, C.nama_kategori, 
                                   ...
                                   LEFT JOIN jabatan I ON I.id_jabatan = D.id_jabatan
                                   LEFT JOIN karyawan K ON K.nik = A.teknisi
                                   WHERE A.id_kondisi IN(3) AND MONTH(tanggal)=MONTH(CURRENT_DATE)
                                   ORDER BY A.tanggal DESC");
        return $query;
    }

    public function all_ticket_prev_datin()
    {
        //Query untuk mendapatkan semua ticket dengan diurutkan berdasarkan tanggal tiket dibuat
        $query = $this->db->query("SELECT A.id_ticket, A.status, A.tanggal, A.last_update, A.id_kondisi, A.deadline, A.teknisi,
                                   A.problem_summary, A.filefoto, B.nama_sub_kategori, C.id_kategori, C.nama_kategori, 
                                   D.nama, F.nama_dept, G.nama_kondisi, G.warna, G.waktu_respon, H.lokasi, I.nama_jabatan, K.nama AS nama_teknisi FROM ticket A 
                                   LEFT JOIN sub_kategori B ON B.id_sub_kategori = A.id_sub_kategori
                                   LEFT JOIN kategori C ON C.id_kategori = B.id_kategori
                                   LEFT JOIN karyawan D ON D.nik = A.reported
                                   ...
                                   LEFT JOIN jabatan I ON I.id_jabatan = D.id_jabatan
                                   LEFT JOIN karyawan K ON K.nik = A.teknisi
                                   WHERE C.id_kategori IN(4) AND MONTH(tanggal)=MONTH(CURRENT_DATE)
                                   ORDER BY A.tanggal DESC");
        return $query;
    }

    public function all_ticket_hsi()
    {
        //Query untuk mendapatkan semua ticket dengan diurutkan berdasarkan tanggal tiket dibuat
        $query = $this->db->query("SELECT A.id_ticket, A.status, A.tanggal, A.last_update, A.id_kondisi, A.deadline, A.teknisi,
                                   A.problem_summary, A.filefoto, B.nama_sub_kategori, C.nama_kategori, 
                                   ...
                                   LEFT JOIN jabatan I ON I.id_jabatan = D.id_jabatan
                                   LEFT JOIN karyawan K ON K.nik = A.teknisi
                                   WHERE A.id_kondisi IN(4)
                                   ORDER BY A.tanggal DESC");
        return $query;
    }

    public function all_ticket_voice()
    {
        //Query untuk mendapatkan semua ticket dengan diurutkan berdasarkan tanggal tiket dibuat
        $query = $this->db->query("SELECT A.id_ticket, A.status, A.tanggal, A.last_update, A.id_kondisi, A.deadline, A.teknisi,
                                   A.problem_summary, A.filefoto, B.nama_sub_kategori, C.nama_kategori, 
                                   ...
                                   LEFT JOIN jabatan I ON I.id_jabatan = D.id_jabatan
                                   LEFT JOIN karyawan K ON K.nik = A.teknisi
                                   WHERE A.id_kondisi IN(5)
                                   ORDER BY A.tanggal DESC");
        return $query;
    }

    public function all_ticket_datin_gaul()
    {
        //Query untuk mendapatkan semua ticket dengan diurutkan berdasarkan tanggal tiket dibuat
        $query = $this->db->query("SELECT A.id_ticket, A.status, A.tanggal, A.last_update, A.id_kondisi, A.deadline, A.teknisi,
                                   A.problem_summary, A.filefoto, B.nama_sub_kategori, C.nama_kategori, 
                                   ...
                                   LEFT JOIN jabatan I ON I.id_jabatan = D.id_jabatan
                                   LEFT JOIN karyawan K ON K.nik = A.teknisi
                                   WHERE problem_summary IN(SELECT problem_summary FROM ticket GROUP BY problem_summary HAVING COUNT(*)>1) 
                                   AND A.id_kondisi IN(1,2,3) AND MONTH(tanggal)=MONTH(CURRENT_DATE)   
                                   ORDER BY A.tanggal DESC");
        return $query;
    }

    public function all_ticket_hsi_gaul()
    {
        //Query untuk mendapatkan semua ticket dengan diurutkan berdasarkan tanggal tiket dibuat
        $query = $this->db->query("SELECT A.id_ticket, A.status, A.tanggal, A.last_update, A.id_kondisi, A.deadline, A.teknisi,
                                   A.problem_summary, A.filefoto, B.nama_sub_kategori, C.nama_kategori, 
                                   ...
                                   LEFT JOIN karyawan K ON K.nik = A.teknisi
                                   WHERE problem_summary IN(SELECT problem_summary FROM ticket GROUP BY problem_summary HAVING COUNT(*)>1) 
                                   AND A.id_kondisi IN(4) AND MONTH(tanggal)=MONTH(CURRENT_DATE)   
                                   ORDER BY A.tanggal DESC");
        return $query;
    }
    public function all_ticket_voice_gaul()
    {
        //Query untuk mendapatkan semua ticket dengan diurutkan berdasarkan tanggal tiket dibuat
        $query = $this->db->query("SELECT A.id_ticket, A.status, A.tanggal, A.last_update, A.id_kondisi, A.deadline, A.teknisi,
                                   A.problem_summary, A.filefoto, B.nama_sub_kategori, C.nama_kategori, 
                                   ...
                                   LEFT JOIN karyawan K ON K.nik = A.teknisi
                                   WHERE problem_summary IN(SELECT problem_summary FROM ticket GROUP BY problem_summary HAVING COUNT(*)>1) 
                                   AND A.id_kondisi IN(5) AND MONTH(tanggal)=MONTH(CURRENT_DATE)   
                                   ORDER BY A.tanggal DESC");
        return $query;
    }
    ...
    public function approve($id)
{
    ...
    { 
        ...
        $query = $this->db->query("SELECT * from absensi where status_absensi = 1");
    ...
    public function dropdown_kondisi()
    {
        ...
        foreach ($query->result() as $row) {
            $value[$row->id_kondisi] = $row->nama_kondisi . "  -  (Process Target " . $row->waktu_respon . " " . "Minutes)";
        }
        return $value;
    }
...
/////////////////////////////////////////////////////////////////////////////////Bagian Teknisi/////////////////////////////////////////////////////////////////////////////////
    ...
    public function daftar_tugas($id)
    {
        ...
        $query = $this->db->query("SELECT A.progress, A.status, A.id_ticket, A.reported, A.tanggal, A.tanggal_solved, A.id_kondisi, A.deadline,
                                   A.problem_detail, A.problem_summary, A.filefoto, B.nama_sub_kategori, 
                                   C.nama_kategori, D.nama, G.nama_kondisi, G.warna, H.lokasi, J.nama_dept, K.nama_jabatan, T.deskripsi, M.rfo, R.rca FROM ticket A 
                                   LEFT JOIN sub_kategori B ON B.id_sub_kategori = A.id_sub_kategori
                                   LEFT JOIN kategori C ON C.id_kategori = B.id_kategori
                                   LEFT JOIN karyawan D ON D.nik = A.reported
    ...
    public function update($id)
    {
        ...
        $rca      = $this->input->post('id_rca');
        $rfo      = $this->input->post('id_rfo');
    ...
    public function changekategori($id)
    {
        ...
        if ($subkat == $sql->id_sub_kategori){
          $this->session->set_flashdata('status1', 'You have not changed the unit!');
...
//////////////////////////////////////////////////////////////////////////////////Bagian User//////////////////////////////////////////////////////////////////////////////////
 ...
 /*   public function getkodeticket()
    ...
    // Method untuk menampilkan data jumlah total tiket bulan ini
    public function get_total_records_bulan_ini() {
      $current_month = date('m');
      $this->db->select('COUNT(*) as total');
      $this->db->from('ticket');
      $this->db->where("MONTH(tanggal)", $current_month);
     //$this->db->where("id_sub_kategori IN(5)");
      $query = $this->db->get();
      $row = $query->row();
      return $row->total;
  }

  // Method untuk menampilkan jumlah data yang sama dalam satu tabel berdasarkan DATIN pada bulan ini
  public function get_total_records_gaul() {
   // $current_month = date('m');
    $this->db->select('COUNT(*) as jml_gaul');
    $this->db->from('ticket');
    $this->db->group_by('problem_summary');
    $this->db->where("id_kondisi IN(1,2,3)");
    ...
    public function get_produktiv() {
      //Mengambil session admin
      $id_user    = $this->session->userdata('id_user');
    $this->db->select('COUNT(id_ticket) as total_produktivitas');
    $this->db->from('ticket');
   // $this->db->join('karyawan', 'ticket.teknisi = karyawan.nik');
    $this->db->where('teknisi="$id_user"');
    //$this->db->having('COUNT(*)>1');
    $this->db->where('MONTH(tanggal)', date('m'));
   ...
   // $total_produktivitas = $result['total_produktivitas'];
    }

// Method untuk menampilkan jumlah data yang sama dalam satu tabel berdasarkan HSI pada bulan ini
public function get_total_records_gaul_hsi() {
  // $current_month = date('m');
   $this->db->select('COUNT(*) as jml_gaul_hsi');
   $this->db->from('ticket');
   $this->db->group_by('problem_summary');
   $this->db->where("id_kondisi IN(4)");
   $this->db->having('COUNT(*)>1');
   $this->db->where('MONTH(tanggal)', date('m'));
   $query = $this->db->get();
   return $query->num_rows();
...
// Method untuk menampilkan jumlah data yang sama dalam satu tabel berdasarkan VOICE pada bulan ini
public function get_total_records_gaul_voice() {
  // $current_month = date('m');
   $this->db->select('COUNT(*) as jml_gaul_voice');
   $this->db->from('ticket');
   $this->db->group_by('problem_summary');
   $this->db->where("id_kondisi IN(5)");
   $this->db->having('COUNT(*)>1');
   $this->db->where('MONTH(tanggal)', date('m'));
   $query = $this->db->get();
   return $query->num_rows();
  ...
  /////////////////////////////////MENAMPILKAN DATA TIKET HSI PER BULAN INI///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  
  // Method untuk menampilkan data jumlah tiket HSI bulan ini
    public function get_total_hsi_bulan_ini() {
      $current_month = date('m');
      $this->db->select('COUNT(*) as total_hsi');
      $this->db->from('ticket');
      $this->db->where("MONTH(tanggal)", $current_month);
      $this->db->where("id_sub_kategori IN(5)");
      $this->db->where("status IN(6,7)");
      $query = $this->db->get();
      $row = $query->row();
      return $row->total_hsi;
  }

  // Method untuk menampilkan data jumlah tiket HSI yang COMPLY periode bulan ini
  public function get_total_hsi_comply_bulan_ini() {
    $current_month = date('m');
    $this->db->select('COUNT(*) as total_hsi_comply');
    $this->db->from('ticket');
    $this->db->where("MONTH(tanggal)", $current_month);
    $this->db->where("status IN(6)");
    $this->db->where("id_sub_kategori IN(5,6,7,8,9,10)");
    $query = $this->db->get();
    $row = $query->row();
    return $row->total_hsi_comply;
  }

  // Method untuk menampilkan data jumlah tiket HSI yang NOT COMPLY periode bulan ini
  public function get_total_hsi_notcomply_bulan_ini() {
    $current_month = date('m');
    $this->db->select('COUNT(*) as total_hsi_notcomply');
    $this->db->from('ticket');
    $this->db->where("MONTH(tanggal)", $current_month);
    $this->db->where("status IN(7)");
    $this->db->where("id_sub_kategori IN(5,6,7,8,9,10)");
    $query = $this->db->get();
    $row = $query->row();
    return $row->total_hsi_notcomply;
  }

/////////////////////////////////MENAMPILKAN DATA TIKET DATIN PER BULAN INI////////////////////////////////////

   // Method untuk menampilkan data jumlah tiket DATIN bulan ini
    public function get_total_datin_bulan_ini() {
      $current_month = date('m');
      $this->db->select('COUNT(*) as total_datin');
      $this->db->from('ticket');
      $this->db->where("MONTH(tanggal)", $current_month);
      $this->db->where("id_sub_kategori IN(3)");
      $query = $this->db->get();
      $row = $query->row();
      return $row->total_datin;
  }

  // Method untuk menampilkan data jumlah tiket DATIN COMPLY bulan ini
  public function get_total_datin_comply_bulan_ini() {
    $current_month = date('m');
    $this->db->select('COUNT(*) as total_datin_comply');
    $this->db->from('ticket');
    $this->db->where("MONTH(tanggal)", $current_month);
    $this->db->where("id_sub_kategori IN(1,2,3,4)");
    $this->db->where("status IN(6)");
    $query = $this->db->get();
    $row = $query->row();
    return $row->total_datin_comply;
  }

  // Method untuk menampilkan data jumlah tiket DATIN NOT COMPLY bulan ini
  public function get_total_datin_notcomply_bulan_ini() {
    $current_month = date('m');
    $this->db->select('COUNT(*) as total_datin_notcomply');
    $this->db->from('ticket');
    $this->db->where("MONTH(tanggal)", $current_month);
    $this->db->where("id_sub_kategori IN(1,2,3,4)");
    $this->db->where("status IN(7)");
    $query = $this->db->get();
    $row = $query->row();
    return $row->total_datin_notcomply;
  }
  
   // Method untuk menampilkan data jumlah tiket DATIN K1 bulan ini
   public function get_total_datin_k1_bulan_ini() {
    $current_month = date('m');
    $this->db->select('COUNT(*) as total_datin_k1');
    $this->db->from('ticket');
    $this->db->where("MONTH(tanggal)", $current_month);
    $this->db->where("id_kondisi IN(1)");
    $this->db->where("status IN(6,7)");
    $query = $this->db->get();
    $row = $query->row();
    return $row->total_datin_k1;
}

  // Method untuk menampilkan data jumlah tiket DATIN K1 COMPLY bulan ini
  public function get_total_datin_k1_comply_bulan_ini() {
    $current_month = date('m');
    $this->db->select('COUNT(*) as total_datin_k1_comply');
    $this->db->from('ticket');
    $this->db->where("MONTH(tanggal)", $current_month);
    $this->db->where("id_kondisi IN(1)");
    $this->db->where("status IN(6)");
    $query = $this->db->get();
    $row = $query->row();
    return $row->total_datin_k1_comply;
}

  // Method untuk menampilkan data jumlah tiket DATIN K1 NOT COMPLY bulan ini
  public function get_total_datin_k1_notcomply_bulan_ini() {
    $current_month = date('m');
    $this->db->select('COUNT(*) as total_datin_k1_notcomply');
    $this->db->from('ticket');
    $this->db->where("MONTH(tanggal)", $current_month);
    $this->db->where("id_kondisi IN(1)");
    $this->db->where("status IN(7)");
    $query = $this->db->get();
    $row = $query->row();
    return $row->total_datin_k1_notcomply;
  }

   // Method untuk menampilkan data jumlah tiket DATIN K2 bulan ini
   public function get_total_datin_k2_bulan_ini() {
    $current_month = date('m');
    $this->db->select('COUNT(*) as total_datin_k2');
    $this->db->from('ticket');
    $this->db->where("MONTH(tanggal)", $current_month);
    $this->db->where("id_kondisi IN(2)");
    $this->db->where("status IN(6,7)");
    $query = $this->db->get();
    $row = $query->row();
    return $row->total_datin_k2;
}

  // Method untuk menampilkan data jumlah tiket DATIN K2 COMPLY bulan ini
  public function get_total_datin_k2_comply_bulan_ini() {
    $current_month = date('m');
    $this->db->select('COUNT(*) as total_datin_k2_comply');
    $this->db->from('ticket');
    $this->db->where("MONTH(tanggal)", $current_month);
    $this->db->where("id_kondisi IN(2)");
    $this->db->where("status IN(6)");
    $query = $this->db->get();
    $row = $query->row();
    return $row->total_datin_k2_comply;
}

  // Method untuk menampilkan data jumlah tiket DATIN K2 NOT COMPLY bulan ini
  public function get_total_datin_k2_notcomply_bulan_ini() {
    $current_month = date('m');
    $this->db->select('COUNT(*) as total_datin_k2_notcomply');
    $this->db->from('ticket');
    $this->db->where("MONTH(tanggal)", $current_month);
    $this->db->where("id_kondisi IN(2)");
    $this->db->where("status IN(7)");
    $query = $this->db->get();
    $row = $query->row();
    return $row->total_datin_k2_notcomply;
  }

  // Method untuk menampilkan data jumlah tiket DATIN K3 bulan ini
  public function get_total_datin_k3_bulan_ini() {
    $current_month = date('m');
    $this->db->select('COUNT(*) as total_datin_k3');
    $this->db->from('ticket');
    $this->db->where("MONTH(tanggal)", $current_month);
    $this->db->where("id_kondisi IN(3)");
    $this->db->where("status IN(6,7)");
    $query = $this->db->get();
    $row = $query->row();
    return $row->total_datin_k3;
}

  // Method untuk menampilkan data jumlah tiket DATIN K3 COMPLY bulan ini
  public function get_total_datin_k3_comply_bulan_ini() {
    $current_month = date('m');
    $this->db->select('COUNT(*) as total_datin_k3_comply');
    $this->db->from('ticket');
    $this->db->where("MONTH(tanggal)", $current_month);
    $this->db->where("id_kondisi IN(3)");
    $this->db->where("status IN(6)");
    $query = $this->db->get();
    $row = $query->row();
    return $row->total_datin_k3_comply;
}

  // Method untuk menampilkan data jumlah tiket DATIN K3 NOT COMPLY bulan ini
  public function get_total_datin_k3_notcomply_bulan_ini() {
    $current_month = date('m');
    $this->db->select('COUNT(*) as total_datin_k3_notcomply');
    $this->db->from('ticket');
    $this->db->where("MONTH(tanggal)", $current_month);
    $this->db->where("id_kondisi IN(3)");
    $this->db->where("status IN(7)");
    $query = $this->db->get();
    $row = $query->row();
    return $row->total_datin_k3_notcomply;
  }

   // Method untuk menampilkan data jumlah tiket VOICE bulan ini
   public function get_total_voice_bulan_ini() {
    $current_month = date('m');
    $this->db->select('COUNT(*) as total_voice');
    $this->db->from('ticket');
    $this->db->where("MONTH(tanggal)", $current_month);
    $this->db->where("id_sub_kategori IN(11,12,13,14,15,16)");
    $this->db->where("status IN(6,7)");
    $query = $this->db->get();
    $row = $query->row();
    return $row->total_voice;
  }

  // Method untuk menampilkan data jumlah tiket DATIN K3 COMPLY bulan ini
  public function get_total_voice_comply_bulan_ini() {
    $current_month = date('m');
    $this->db->select('COUNT(*) as total_voice_comply');
    $this->db->from('ticket');
    $this->db->where("MONTH(tanggal)", $current_month);
    $this->db->where("id_sub_kategori IN(11,12,13,14,15,16)");
    $this->db->where("status IN(6)");
    $query = $this->db->get();
    $row = $query->row();
    return $row->total_voice_comply;
}

  // Method untuk menampilkan data jumlah tiket DATIN K3 NOT COMPLY bulan ini
  public function get_total_voice_notcomply_bulan_ini() {
    $current_month = date('m');
    $this->db->select('COUNT(*) as total_voice_notcomply');
    $this->db->from('ticket');
    $this->db->where("MONTH(tanggal)", $current_month);
    $this->db->where("id_sub_kategori IN(11,12,13,14,15,16)");
    $this->db->where("status IN(7)");
    $query = $this->db->get();
    $row = $query->row();
    return $row->total_voice_notcomply;
  }
    ...
    public function getMyTicketFilter($id, $start_date, $end_date){
      $query = $this->db->query("SELECT A.progress, A.status, A.id_ticket, A.reported, A.tanggal, A.id_kondisi, A.deadline, A.last_update,
                                   A.problem_detail, A.problem_summary, A.filefoto, B.nama_sub_kategori, A.teknisi,
                                   C.nama_kategori, D.nama, G.nama_kondisi, G.warna, H.lokasi, J.nama_dept, K.nama AS nama_teknisi FROM ticket A 
                                   LEFT JOIN sub_kategori B ON B.id_sub_kategori = A.id_sub_kategori
                                   ...
                                   LEFT JOIN departemen J ON J.id_dept = I.id_dept
                                   LEFT JOIN karyawan K ON K.nik = A.teknisi
                                   WHERE A.reported = '$id' AND 
                                   A.tanggal BETWEEN ? AND ? ORDER BY A.tanggal DESC", array($start_date, $end_date));
        return $query;
    }
    ...
    public function dropdown_rca(){
      $sql = "SELECT * FROM RCA";
      $query = $this->db->query($sql);
      $value[''] = '--CHOOSE--';
      foreach($query->result() as $row){
          $value[$row->id_rca] = $row->rca;
      }
      return $value;
    }
    public function get_selected_rca($id){
      $sql = "SELECT id_rca from RCA WHERE id_rca = '$id'";
      $query = $this->db->query($sql);
      $result = $query->row();
      return $result;
    }

    public function dropdown_rfo(){
      $sql = "SELECT * FROM RFO ORDER BY RFO";
      $query = $this->db->query($sql);
      $value[''] = '--CHOOSE--';
      foreach($query->result() as $row){
        $value[$row->id_rfo] = $row->rfo;
      }
      return $value;
    }
    public function get_selected_rfo($id){
      $sql = "SELECT id_rfo from RFO WHERE id_rfo = '$id'";
      $query = $this->db->query($sql);
      $result = $query->row();
      return $result;
    }
...
////////////////////////////////////////////////////////////////////////////////Bagian Dashboard////////////////////////////////////////////////////////////////////////////////
    ...
    public function getTicketFilter($start_date, $end_date){
      $query = $this->db->query("SELECT A.id_ticket, A.status, A.tanggal, A.last_update, A.tanggal_solved, 
                                  A.id_kondisi, A.deadline, A.teknisi, A.problem_summary, A.filefoto, B.nama_sub_kategori,
                                  C.nama_kategori, D.nama, F.nama_dept, G.nama_kondisi, G.warna, G.waktu_respon, H.lokasi, 
                                  I.nama_jabatan, K.nama AS nama_teknisi, 
    ...
    {
        $query = $this->db->query("SELECT A.id_user, B.nama, B.nik, COUNT(C.status IN (3)) as total FROM user A 
                                    LEFT JOIN karyawan B ON B.nik = A.username 
                                    LEFT JOIN tracking C ON C.id_user = B.nik 
                                    WHERE A.level = 'Admin' AND DATE(tanggal)=DATE(CURRENT_DATE) 
                                    GROUP BY B.nama ");
        return $query;
    }
    ...
    public function getTl()
    {
        $query = $this->db->query("SELECT A.id_user, B.nama, B.nik, COUNT(C.status IN (3)) as total FROM user A 
                                    LEFT JOIN karyawan B ON B.nik = A.username 
                                    LEFT JOIN tracking C ON C.id_user = B.nik 
                                    WHERE A.level = 'User' AND DATE(tanggal)=DATE(CURRENT_DATE) 
                                    GROUP BY B.nama ");
        return $query;
    }
    ...
    public function getHd_pro()
    {
        $query = $this->db->query("SELECT A.id_user, B.nama, B.nik, COUNT(C.status IN (3)) as total FROM user A 
                                    LEFT JOIN karyawan B ON B.nik = A.username 
                                    LEFT JOIN tracking C ON C.id_user = B.nik 
                                    WHERE A.level = 'Admin' AND MONTH(tanggal)=MONTH(CURRENT_DATE) 
                                    GROUP BY B.nama ");
        return $query;
    }

    public function getTl_pro()
    {
        $query = $this->db->query("SELECT A.id_user, B.nama, B.nik, COUNT(C.status IN (3)) as total FROM user A 
                                    LEFT JOIN karyawan B ON B.nik = A.username 
                                    LEFT JOIN tracking C ON C.id_user = B.nik 
                                    WHERE A.level = 'User' AND MONTH(tanggal)=MONTH(CURRENT_DATE) 
                                    GROUP BY B.nama ");
        return $query;
    }
    ...
    //Method untuk mengambil data semua user dengan level 'Helpdesk'
    public function getHelpdesk(){
        //Query untuk mengambil data semua user dengan level 'Helpdesk'
        $this->db->where('level', 'Helpdesk');
        return $this->db->get('helpdesk');
    }
...
////////////////////////////////////////////////////////////////////////////////Bagian Statistik////////////////////////////////////////////////////////////////////////////////
    ...
    {
        $query = $this->db->query("SELECT MONTH (tanggal) AS tahun, COUNT(*) AS jumtahun FROM ticket
...
/////////////////////////////////////////////////////////////////////////////Bagian Notifikasi Email/////////////////////////////////////////////////////////////////////////////
    public function emailbuatticket($id)
    {
        ...
        $isiEmail .= "<div>Please respone and set the priority of the ticket in <b>BGES SBS</b></div>";
        $isiEmail .= '<div>
                        <table>
                          <tbody>
                            <tr>
        ...
        $this->email->from('ccan.asr.sbs@gmail.com', $query->nama); 
        $this->email->to('ccan.asr.sbs@gmail.com');   
    ...
    public function emailapprove($id)
    {
        ...
        $isiEmail .= "<div>Your ticket will be processed according to predetermined priorities. You can track your ticket in <b>Dashboard BGES</b></div>";
        $isiEmail .= '<div>
                        <table>
                          <tbody>
                            <tr>
        ...
        $this->email->from('ccan.asr.sbs@gmail.com', 'BGES');   
    ...
    public function emailtugas($id)
    {
        ...
        $isiEmail .= "<div>Pelase check your <code>Ticket Assigned</code> menu on <b>Dashboard BGES</b></div>";
        $isiEmail .= '<div>
                        <table>
                          <tbody>
                            <tr>
    ...
    public function emaildiproses($id)
    {
        ...
        $isiEmail .= "<div>Dear ".$query->nama."</div>";
        $isiEmail .= "<div>You can track your ticket in <b>Dashboard BGES</b></div>";
        $isiEmail .= '<div>
                        <table>
                          <tbody>
                            <tr>
    ...
    public function emailubah($id)
    {
        ...
        $isiEmail  = "<h1>Category of Tiket (".$query->id_ticket.") Has Been Changed</h1>";
        ...
        $this->email->set_newline("\r\n");  
        $this->email->from('ccan.asr.sbs@gmail.com', $query->nama_teknisi);   
        $this->email->to('ccan.asr.sbs@gmail.com');   
